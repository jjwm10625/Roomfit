...
class ChatViewModel4(application: Application) : AndroidViewModel(application) {
    ...
    private fun loadMessages(): List<Pair<String, Boolean>> {
        val jsonString = sharedPreferences.getString("messages_chat4", "[]")
    ...
    private fun saveMessages(messages: List<Pair<String, Boolean>>) {
        ...
        sharedPreferences.edit().putString("messages_chat4", jsonArray.toString()).apply()
    }
}