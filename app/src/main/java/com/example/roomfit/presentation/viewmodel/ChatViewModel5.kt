...
class ChatViewModel5(application: Application) : AndroidViewModel(application) {
    ...
    private fun loadMessages(): List<Pair<String, Boolean>> {
        val jsonString = sharedPreferences.getString("messages_chat5", "[]")
    ...
    private fun saveMessages(messages: List<Pair<String, Boolean>>) {
        ...
        sharedPreferences.edit().putString("messages_chat5", jsonArray.toString()).apply()
    }
}